...
use dexter::multi_staking::{
    ExecuteMsg, InstantLpUnlockFee, InstantUnbondConfig, QueryMsg, UnbondConfig, UnlockFeeTier,
};
use utils::{
    instantiate_multi_staking_contract, store_lp_token_contract, store_multi_staking_contract,
};

use crate::utils::{
    assert_user_lp_token_balance, bond_lp_tokens, create_lp_token, instant_unbond_lp_tokens,
    instant_unlock_lp_tokens, mint_lp_tokens_to_addr, mock_app, query_token_locks,
    unbond_lp_tokens,
};
pub mod utils;
...
fn test_instant_unbond_and_unlock() {
    ...
    let query = QueryMsg::UnbondConfig {
        lp_token: Some(lp_token_addr_2.clone()),
    };
    let res: UnbondConfig = app
        .wrap()
        .query_wasm_smart(multi_staking_instance.clone(), &query)
        .unwrap();

    assert_eq!(res, unbond_config);

    // query fee tiers and validate that we get error in the query
    let query = QueryMsg::InstantUnlockFeeTiers {
        lp_token: lp_token_addr_2.clone(),
    };
    let res: Result<Vec<UnlockFeeTier>, _> = app
        .wrap()
        .query_wasm_smart(multi_staking_instance.clone(), &query);
    ...
    assert_eq!(
        ...
        "Generic error: Querier contract error: Instant unbond/unlock is disabled for this LP"
    );
    ...
    // assert error is that ILPU is disabled
    assert_eq!(
        res.err().unwrap().root_cause().to_string(),
        "Instant unbond/unlock is disabled for this LP"
    );
    ...
    let res: InstantLpUnlockFee = app
        .wrap()
        .query_wasm_smart(multi_staking_instance.clone(), &query)
        .unwrap();

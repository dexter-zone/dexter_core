...
impl<'a> CreatePoolTestSuite<'a> {
    fn new(test_setup: &'a GovAdminTestSetup) -> Self {
        ...
        let valid_reward_schedules =
            vec![dexter::governance_admin::RewardScheduleCreationRequest {
                ...
                end_block_time: current_block_time + 20 * 24 * 60 * 60,
            }];
        ...
        let total_funds_to_send = vec![
            // proposal deposit + bootstrapping amount + pool creation fee
            Coin::new(22000000, xprt_asset_info.denom().unwrap()),
        ];
    ...
    fn run_all(&self) {
        ...
        println!("Running success case: Create Pool with a CW20 Pool Creation Fee");
        self.test_success_create_pool_with_cw20_pool_creation_fee();
        ...
        assert_eq!(
            ...
            vec![
                ...
                Asset::new(
                    ...
                    Uint128::from(22000000u128)
                ),
            ]
        );

        // valid deposit breakdown
        assert_eq!(
            funds_for_pool_creation.deposit_breakdown,
            vec![
                UserDeposit {
                ...
                UserDeposit {
                    category: dexter::governance_admin::FundsCategory::PoolCreationFee,
                    assets: vec![Asset::new(
                        AssetInfo::native_token("uxprt".to_string()),
                        Uint128::from(10000000u128)
                    )]
    ...
    fn test_failure_with_insufficient_native_funds(&self) {
        ...
        self.test_setup.increase_cw20_allowance(
            &self.test_setup.cw20_token_1,
            &self.test_setup.gov_admin_instance,
            Uint128::from(1000000u128),
            &self.user,
        );
        ...
        assert_eq!(error.to_string(), "execute error: failed to execute message; message index: 0: Insufficient funds sent for pool creation for uxprt - Amount Sent: 0 - Needed Amount: 22000000: execute wasm contract failed");

        self.test_setup.decrease_cw20_allowance(
            &self.test_setup.cw20_token_1,
            &self.test_setup.gov_admin_instance,
            Uint128::from(1000000u128),
            &self.user,
    ...
    fn test_success_create_pool_with_cw20_pool_creation_fee(&self) {
        let wasm = Wasm::new(self.persistence);
        let valid_create_pool_msg =
            dexter::governance_admin::ExecuteMsg::CreatePoolCreationProposal {
                proposal_description: self.proposal_description.clone(),
        ...
        // update pool creation fee to be in CW20 tokens
        let update_config_msg = dexter::vault::ExecuteMsg::UpdateConfig {
            lp_token_code_id: None,
            fee_collector: None,
            pool_creation_fee: Some(PoolCreationFee::Enabled {
                fee: Asset::new_token(
                    self.test_setup.cw20_token_1.clone(),
                    Uint128::from(1000000u128),
                ),
            }),
        ...
        let cosmos_msg = CosmosMsg::Wasm(cosmwasm_std::WasmMsg::Execute {
            contract_addr: self.test_setup.vault_instance.to_string(),
            msg: to_json_binary(&update_config_msg).unwrap(),
            funds: vec![],
        });
        ...
        // verify that the pool creation fee has been updated
        let get_config_query = dexter::vault::QueryMsg::Config {};
        let config: dexter::vault::ConfigResponse = wasm
            .query(
                &self.test_setup.vault_instance.to_string(),
                &get_config_query,
            )
        ...
        let valid_funds = vec![
            // proposal deposit + bootstrapping amount + reward schedule amount
            // pool creation fee is CW20 so it's not added here
            Coin::new(12000000, "uxprt"),
        ];
        ...
        // this would fail as the CW20 token has not been approved
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert_eq!(error.to_string(), "execute error: failed to execute message; message index: 0: Insufficient spend limit for token persistence1mf6ptkssddfmxvhdx0ech0k03ktp6kf9yk59renau2gvht3nq2gqtu9smg - Current approval: 0 - Needed Approval: 2000000: execute wasm contract failed");
        ...
        // directly call the resume create pool function to simulate the proposal passing
        // let resume_create_pool_msg = dexter::governance_admin::ExecuteMsg::ResumeCreatePool {
        //     pool_creation_request_id: 2,
        // };
        ...
        // revert back the pool creation fee to native tokens
        let update_config_msg = dexter::vault::ExecuteMsg::UpdateConfig {
            lp_token_code_id: None,
            fee_collector: None,
            pool_creation_fee: Some(PoolCreationFee::Enabled {
                fee: Asset::new_native("uxprt".to_string(), Uint128::from(10000000u128)),
            }),
            auto_stake_impl: None,
            paused: None,
            reward_schedule_validation_assets: None,
    ...
    fn test_valid_refund_amount_post_successful_pool_creation(&self) {
        ...
        assert_eq!(error.to_string(), "execute error: failed to execute message; message index: 0: Funds already claimed for this request at block height: 35: execute wasm contract failed");
    }
    ...
    fn test_refund_for_rejected_proposal(&self) {
        ...
        assert_eq!(
            ...
            vec![
                ...
                Asset::new(xprt_asset.clone(), Uint128::from(22000000u128))
            ]
        );
    ...
    fn test_refund_for_vetoed_proposal(&self) {
        ...
        // query for the refund of the deposit amount
        let query_refund_msg = QueryMsg::RefundableFunds {
            request_type: GovAdminProposalRequestType::PoolCreationRequest { request_id: 4 },
        };
        ...
        // verify that the refundable funds are equal to the pool creation fee + reward schedule amount + pool bootstrapping amount + pool creation fee
        assert_eq!(
            refundable_funds.refund_amount,
            vec![
                Asset::new(cw20_asset.clone(), Uint128::from(1000000u128)),

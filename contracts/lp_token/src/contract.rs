...
fn verify_xml_preamble(data: &[u8]) -> Result<(), ContractError> {
    // The easiest way to perform this check would be just match on regex, however regex  compilation is heavy and probably not worth it.
    let preamble = data
...
/// ## Description -  Creates a new contract with the specified parameters in the [`InstantiateMsg`].

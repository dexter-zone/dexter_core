...
pub fn calculate_pow(base: Decimal, exp: Decimal, precision: Option<Decimal>) -> StdResult<Decimal> {
    ...
    println!("\ncalculate_pow FN CALLED :: base: {}, exp: {}, precision: {}", base, exp, precision);
	...
	if base > Decimal::from_str("2").unwrap()  {
        return Err(StdError::generic_err("calculate_pow : base must be less than 2"));
	}
    ...
    let fractional =  Decimal::from_atomics(exp.atomics() % DECIMAL_FRACTIONAL, 18 ).map_err(|e| StdError::generic_err(e.to_string()))?;
    println!("exp: {}, integer: {}, fractional: {}", exp, integer, fractional);
...
mod tests {
    ...
    fn check_approx_pow() {
        ...
        assert_eq!(&res.as_ref().unwrap().clone().to_string()[0..10], "1.74603121");
        ...
        assert_eq!(&res.as_ref().unwrap().clone().to_string()[0..11], "1.186212638");        
        ...
        assert_eq!(&res.as_ref().unwrap().clone().to_string()[0..11], "0.839898055");      
